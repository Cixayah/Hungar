...
class QueueView(View):
    ...
    async def update_message(self):
        try:
            ...
            queue_message = "\n".join(
                [f"{i+1}. {track}" for i, track in enumerate(tracks, start=start)]
            )
            ...
            await self.interaction.edit_original_response(
                content="Erro ao atualizar a fila de músicas.", view=self
            )
...
class MusicBot(commands.Cog):
    ...
    async def play_next(self, interaction: discord.Interaction):
        if (
            ...
            or not interaction.guild.voice_client.is_connected()
        ):
            await self.cleanup()
            return
        ...
        if self.queue:
            try:
                ...
                url = (
                    track_info
                    if track_info.startswith("http")
                    else f"ytsearch:{track_info}"
                )
                ...
                while retries > 0:
                    try:
                        ...
                        if retries == 0:
                            await interaction.channel.send(
                                "Não foi possível reproduzir esta música após várias tentativas."
                            )
                            self.is_playing = False
                ...
                await interaction.channel.send(
                    "Ocorreu um erro ao reproduzir esta música. Pulando para a próxima..."
                )
                self.is_playing = False
                await self.play_next(interaction)
        else:
        ...
        if not interaction.guild.voice_client:
            ...
            try:
                ...
                if self.reconnect_attempts < self.max_reconnect_attempts:
                    ...
                    raise Exception(
                        "Não foi possível conectar ao canal de voz após várias tentativas."
                    )

    @app_commands.command(
        name="play", description="Reproduz uma música do YouTube ou Spotify."
    )
        ...
        try:
            ...
            if track_info is None:
                await interaction.followup.send(
                    "Não consegui buscar essa música ou playlist.", ephemeral=True
                )
                return

            self.queue.extend(track_info)
            await interaction.followup.send(
                f"Adicionadas à fila: **{len(track_info)}** músicas."
            )
    ...
    @app_commands.command(
        name="invertqueue",
        description="Toca a fila de músicas da última até a primeira.",
    )
    async def invert_queue(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.followup.send(
                f"A fila foi invertida. Agora tocando da última música até a primeira."
            )
    ...
    @app_commands.command(
        name="stop", description="Para a música atual e limpa a fila."
    )
        ...
        try:
            if interaction.guild.voice_client:
                ...
                await interaction.response.send_message(
                    "O bot não está tocando música no momento."
                )
        except Exception as e:
            print(f"Erro ao parar música: {e}")
            await self.cleanup()
            await interaction.response.send_message(
                "Houve um erro ao parar, mas o estado foi limpo."
            )
    ...
    async def leave(self, interaction: discord.Interaction):
        try:
            ...
            await self.cleanup()
            await interaction.response.send_message(
                "Houve um erro ao sair, mas o estado foi limpo."
            )
    ...
    async def reconnect(self, interaction: discord.Interaction):
        try:
            ...
            await interaction.followup.send(
                "Erro ao tentar reconectar. Por favor, tente novamente."
            )
    ...
    async def queue(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            content="Aqui está a fila de músicas.", view=view, ephemeral=True
        )
